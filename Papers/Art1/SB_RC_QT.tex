...
To provide concrete examples we now turn to the application of our ideas to two specific cases.  We first consider the pure diffusing limit, $S = \sigma_{z}$,\todo[]{put here some relevant points here.} and conclude with the dissipative spin-boson model, $S = \sigma_{x}$. \todo[]{put here some relevant points here.} For both models $S^2 =1$ and the renormalization term in \eqref{eq:H_SB} becomes a trivial shift. We take an ohmic spectral density $J^{(1)}(w) = \gamma \omega e^{- \omega / \Lambda}$ for the residual environment, which in the limit $\Lambda \to \infty$ corresponds to the spin-boson spectral density (see \eqref{eq:J0toJ1})
\begin{equation}
\label{eq:J0DL} 
J^{(0)}(\omega) = \frac{ \alpha \omega_c \omega}{(\omega_c - \frac{\omega \omega_c}{\Omega_{0}} )^2 + \omega^2 },
\end{equation}
with $\alpha =  4 g^2 / \Omega_{0}$ and $\omega_c = \Omega_{0} / \gamma$. Notice that in the overdamped limit, $\Omega_{0} \ll \gamma$,  spectral density \eqref{eq:J0DL} is of the Drude-Lorentz form \cite{IlesSmith:2014db}, which allows us to benchmark our results against data attained from the numerically exact HEOM.

\todo[inline]{We should select a space of parameters in a given region. The paper of Thoss might be a reference to look for this.}
