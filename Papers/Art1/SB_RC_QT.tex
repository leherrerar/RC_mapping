...
We map the system \todo[]{Talk about the Bogoliubov transforms} into a model where a collective mode of the environment, denoted by the creation (annihilation) operator $b$ ($b^{\dag}$), is incorporated into an effective system Hamiltonian coupled to a residual environment \todo[]{Luis: please write the appendix explaining how to get the reaction coordinate}(see Appendix~\ref{A:RCmap}). The spin-boson hamiltonian decomposes into
\begin{equation}
H = H_{S} + H_{B} + H_{I},
\end{equation}
